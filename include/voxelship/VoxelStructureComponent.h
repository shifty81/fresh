...
public:
    ...
    const std::vector<VoxelBlock>& getBlocks() const { return blocks; }
    size_t getBlockCount() const { return blocks.size(); }
    const VoxelBlock& getBlock(size_t index) const { return blocks[index]; }
    ...
    // Collision detection
    bool checkCollision(const VoxelStructureComponent& other, 
                       const glm::vec3& thisPos, const glm::vec3& otherPos) const;
