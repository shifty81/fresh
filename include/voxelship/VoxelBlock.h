...
public:
    ...
    const VoxelMaterial& getMaterial() const { return material; }
    float getMass() const { return material.mass * (size.x * size.y * size.z); }
    
    // Setters
    void setPosition(const glm::vec3& pos) { position = pos; }
    void setSize(const glm::vec3& sz) { size = sz; }
    void setMaterial(const VoxelMaterial& mat) { material = mat; }
