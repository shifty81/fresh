...
public:
    ...
    const std::vector<AssetMetadata>& getAssets() const { return assets; }
...
public:
     ...
     */
    std::vector<AssetMetadata> queryAssets(
        BiomeType biome = BiomeType::Any,
        const std::string& tag = "",
        PlacementRule rule = PlacementRule::Random
    ) const;
     ...
     */
    std::vector<AssetInstance> generateAssetsForChunk(
        VoxelWorld* world,
        int chunkX,
        int chunkZ,
        uint32_t seed
     ...
     */
    void placeAssetsInWorld(const std::vector<AssetInstance>& instances,
                           VoxelWorld* world);
...
private:
    ...
    std::vector<AssetInstance> placeRandomly(
        const AssetMetadata& asset, VoxelWorld* world,
        int chunkX, int chunkZ, uint32_t seed);
    
    std::vector<AssetInstance> placeClustered(
        const AssetMetadata& asset, VoxelWorld* world,
        int chunkX, int chunkZ, uint32_t seed);
    
    std::vector<AssetInstance> placeGrid(
        const AssetMetadata& asset, VoxelWorld* world,
        int chunkX, int chunkZ, uint32_t seed);
    
    bool canPlaceAsset(const AssetMetadata& asset, const glm::vec3& position,
