...
void NativeMainMenu::applyDarkTheme(HWND hwnd)
{
    ...
    SetClassLongPtr(hwnd, GCLP_HBRBACKGROUND, 
                   (LONG_PTR)CreateSolidBrush(UnrealColors::WindowBg));
}
...
void NativeMainMenu::applyDarkThemeToControl(HWND control)
{
    ...
    SetClassLongPtr(control, GCLP_HBRBACKGROUND,
                   (LONG_PTR)CreateSolidBrush(UnrealColors::InputBg));
}
...
void NativeMainMenu::scanWorldSaves()
{
    ...
    try {
        ...
        LOG_ERROR_C("Failed to scan saves directory: " + std::string(e.what()), 
                   "NativeMainMenu");
    }
    
    LOG_INFO_C("Found " + std::to_string(m_worldSaves.size()) + " saved worlds", 
              "NativeMainMenu");
}

INT_PTR CALLBACK NativeMainMenu::mainMenuDialogProc(HWND hwnd, UINT msg, 
...
void NativeMainMenu::createMainMenuDialog(HWND hwnd)
{
    ...
    // Title
    HWND title = CreateWindowEx(
        0, L"STATIC", L"FRESH VOXEL ENGINE",
        WS_CHILD | WS_VISIBLE | SS_CENTER,
        150, 30, 300, 40,
        hwnd, (HMENU)ID_STATIC_TITLE, GetModuleHandle(nullptr), nullptr);
    ...
    CreateWindowEx(
        ...
        hwnd, (HMENU)ID_BTN_CREATE_WORLD, GetModuleHandle(nullptr), nullptr);
    
    buttonY += buttonHeight + spacing;
    
    // Load World button
    CreateWindowEx(
        0, L"BUTTON", L"Load Existing World",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        centerX, buttonY, buttonWidth, buttonHeight,
        hwnd, (HMENU)ID_BTN_LOAD_WORLD, GetModuleHandle(nullptr), nullptr);
    ...
    CreateWindowEx(
        ...
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        centerX, buttonY, buttonWidth, buttonHeight,
        hwnd, (HMENU)ID_BTN_EXIT, GetModuleHandle(nullptr), nullptr);
    
    // Version info at bottom
    CreateWindowEx(
        0, L"STATIC", L"Version 0.1.0 - C++20 Voxel Engine",
        WS_CHILD | WS_VISIBLE | SS_CENTER,
        100, 350, 400, 20,
        hwnd, nullptr, GetModuleHandle(nullptr), nullptr);
}

INT_PTR CALLBACK NativeMainMenu::createWorldDialogProc(HWND hwnd, UINT msg,
                                                        WPARAM wParam, LPARAM lParam)
{
    NativeMainMenu* menu = nullptr;
...
void NativeMainMenu::createWorldCreationDialog(HWND hwnd)
{
    ...
    // World Name input
    CreateWindowEx(
        WS_EX_CLIENTEDGE, L"EDIT", L"New World",
        WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL,
        controlX, y, controlWidth, 24,
        hwnd, (HMENU)ID_EDIT_WORLD_NAME, GetModuleHandle(nullptr), nullptr);
    
    y += 40;
    
    // World Seed label
    CreateWindowEx(
        0, L"STATIC", L"World Seed:",
        WS_CHILD | WS_VISIBLE,
        labelX, y, 120, 20,
        hwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    CreateWindowEx(
        ...
        controlX, y, controlWidth, 24,
        hwnd, (HMENU)ID_EDIT_SEED, GetModuleHandle(nullptr), nullptr);
    
    y += 30;
    
    CreateWindowEx(
        0, L"STATIC", L"(Leave empty for random)",
        WS_CHILD | WS_VISIBLE,
        controlX, y, 300, 20,
        hwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    HWND radio3D = CreateWindowEx(
        0, L"BUTTON", L"3D World (Full Voxel)",
    ...
    CreateWindowEx(
        0, L"BUTTON", L"2D World",
        WS_CHILD | WS_VISIBLE | BS_AUTORADIOBUTTON,
        controlX, y, 300, 20,
        hwnd, (HMENU)ID_RADIO_2D, GetModuleHandle(nullptr), nullptr);
    ...
    CreateWindowEx(
        ...
        centerX, y, buttonWidth, buttonHeight,
        hwnd, (HMENU)ID_BTN_CREATE, GetModuleHandle(nullptr), nullptr);
    
    CreateWindowEx(
        0, L"BUTTON", L"Cancel",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        centerX + buttonWidth + 20, y, buttonWidth, buttonHeight,
        hwnd, (HMENU)ID_BTN_CANCEL, GetModuleHandle(nullptr), nullptr);
}

INT_PTR CALLBACK NativeMainMenu::loadWorldDialogProc(HWND hwnd, UINT msg,
                                                      WPARAM wParam, LPARAM lParam)
{
    NativeMainMenu* menu = nullptr;
    ...
    case WM_COMMAND: {
        ...
        case ID_BTN_LOAD: {
            ...
            if (sel != LB_ERR) {
                ...
                LOG_INFO_C("Loading world: " + 
                          std::string(menu->m_loadWorldName.begin(), 
...
void NativeMainMenu::createWorldLoadingDialog(HWND hwnd)
{
    ...
    // Title
    CreateWindowEx(
        0, L"STATIC", L"Select a world to load:",
        WS_CHILD | WS_VISIBLE,
        20, 20, 460, 20,
        hwnd, nullptr, GetModuleHandle(nullptr), nullptr);
    ...
    HWND listBox = CreateWindowEx(
        ...
        20, 50, 460, 250,
        hwnd, (HMENU)ID_LIST_WORLDS, GetModuleHandle(nullptr), nullptr);
    ...
    CreateWindowEx(
        ...
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | BS_DEFPUSHBUTTON,
        centerX, buttonY, buttonWidth, buttonHeight,
        hwnd, (HMENU)ID_BTN_LOAD, GetModuleHandle(nullptr), nullptr);
    
    CreateWindowEx(
        0, L"BUTTON", L"Cancel",
        WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON,
        centerX + buttonWidth + 20, buttonY, buttonWidth, buttonHeight,
        hwnd, (HMENU)ID_BTN_CANCEL, GetModuleHandle(nullptr), nullptr);
}
