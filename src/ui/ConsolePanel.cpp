...
void ConsolePanel::renderMessages()
{
    ...
    while (clipper.Step()) {
        for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; i++) {
            ...
            ImGui::TextWrapped("[%s] %s: %s", msg.timestamp.c_str(), prefix, msg.message.c_str());
            ImGui::PopStyleColor();
        }
    }
}
...
void ConsolePanel::executeCommand(const std::string& commandLine)
{
    ...
    if (it != m_commands.end()) {
        try {
            ...
            addMessage(ConsoleMessageType::Error, "Command error: " + std::string(e.what()));
        }
    } else {
...
void ConsolePanel::registerDefaultCommands()
{
        ...
        [this](const std::vector<std::string>& args) {
            if (args.empty()) {
                ...
                for (const auto& cmd : m_commands) {
                    std::string desc =
                        cmd.second.description.empty() ? "No description" : cmd.second.description;
                ...
                if (it != m_commands.end()) {
                    addMessage(ConsoleMessageType::Info, args[0] + ": " + it->second.description);
                } else {
    ...
    // Clear command
    registerCommand(
        "clear", [this](const std::vector<std::string>&) { clear(); },
