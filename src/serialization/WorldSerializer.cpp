...
bool WorldSerializer::saveWorld(const VoxelWorld* world, const std::string& filename) {
    ...
    try {
        ...
        for (const auto& [pos, chunk] : world->getChunks()) {
            ...
            for (int y = 0; y < CHUNK_HEIGHT; ++y) {
                for (int z = 0; z < CHUNK_SIZE; ++z) {
                    for (int x = 0; x < CHUNK_SIZE; ++x) {
                        ...
                        file.write(reinterpret_cast<const char*>(&voxel.light), sizeof(voxel.light));
                    }
                }
            }
            ...
            savedChunks++;
            if (savedChunks % 10 == 0) {
                std::cout << "  Saved " << savedChunks << "/" << chunkCount << " chunks..." << std::endl;
            }
        }
...
bool WorldSerializer::loadWorld(VoxelWorld* world, const std::string& filename) {
    ...
    try {
        ...
        for (uint32_t i = 0; i < chunkCount; ++i) {
            ...
            if ((i + 1) % 10 == 0) {
                std::cout << "  Loaded " << (i + 1) << "/" << chunkCount << " chunks..." << std::endl;
            }
        }
