...
std::string GalaxyGenerator::generateStationName(int x, int y, std::mt19937& rng) {
    std::vector<std::string> prefixes = {
        "Alpha", "Beta", "Gamma", "Delta", "Epsilon", "Zeta", "Nova", "Stellar"
    };
    std::vector<std::string> suffixes = {
        "Station", "Outpost", "Haven", "Port", "Hub", "Terminal", "Base"
    };
    ...
    oss << prefixes[prefixDist(rng)] << " " << suffixes[suffixDist(rng)] 
        << " " << x << "-" << y;
...
std::string GalaxyGenerator::getRandomFaction(std::mt19937& rng) {
    std::vector<std::string> factions = {
        "Independent", "Federation", "Empire", "Republic", "Alliance", "Pirates", "Merchants"
    };
