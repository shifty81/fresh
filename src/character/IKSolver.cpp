...
IKSolution TwoBoneIK::solveWithConstraints(
    const glm::vec3& origin,
    float joint1Length,
    float joint2Length,
    const glm::vec3& target,
...
FootIK::FootIK() 
    : hipToKneeLength(2.0f)
    , kneeToFootLength(2.0f)
    , maxStretch(1.2f)
    , enabled(true)
{
}
...
void FootIK::setLegLengths(float hipToKneeLength, float kneeToFootLength) {
    this->hipToKneeLength = hipToKneeLength;
    this->kneeToFootLength = kneeToFootLength;
}

IKSolution FootIK::solveFoot(
    ...
    const glm::vec3& surfaceNormal)
{
    ...
    // Solve two-bone IK
    solution = TwoBoneIK::solve(
        hipPosition,
        hipToKneeLength,
        kneeToFootLength,
        hipPosition + toTarget,
        poleVector
    );
...
HandIK::HandIK() 
    : shoulderToElbowLength(1.5f)
    , elbowToHandLength(1.5f)
    , enabled(true)
{
}
...
void HandIK::setArmLengths(float shoulderToElbowLength, float elbowToHandLength) {
    this->shoulderToElbowLength = shoulderToElbowLength;
    this->elbowToHandLength = elbowToHandLength;
}

IKSolution HandIK::solveHand(
...
glm::vec3 LookAtIK::calculateLookRotation(
...
glm::vec3 LookAtIK::calculateLookRotationConstrained(
    const glm::vec3& headPosition,
    const glm::vec3& targetPosition,
    const glm::vec3& upVector,
    float maxYaw,
