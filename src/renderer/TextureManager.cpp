...
std::shared_ptr<Texture> TextureManager::createTexture(const std::string& name,
    ...
    for (const auto& pair : textureCache) {
        if (pair.second && pair.second->isValid()) {
            ...
            switch (fmt) {
                ...
                case TextureFormat::Depth24: bytesPerPixel = 3; break;
                case TextureFormat::Depth32F: bytesPerPixel = 4; break;
            }
