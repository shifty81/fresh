...
public:
    ...
    void* getNativeHandle() override { return reinterpret_cast<void*>(static_cast<uintptr_t>(buffer)); }
...
public:
    ...
    void* getNativeHandle() override { return reinterpret_cast<void*>(static_cast<uintptr_t>(texture)); }
...
public:
    ...
    void* getNativeHandle() override { return reinterpret_cast<void*>(static_cast<uintptr_t>(program)); }
...
OpenGLRenderContext::OpenGLRenderContext()
    : window(nullptr), width(0), height(0), defaultVAO(0) {
}
...
bool OpenGLRenderContext::initialize(Window* win) {
    ...
    LOG_INFO_C("OpenGL Vendor: " + std::string(reinterpret_cast<const char*>(vendor)), "OpenGLRenderContext");
    LOG_INFO_C("OpenGL Renderer: " + std::string(reinterpret_cast<const char*>(renderer)), "OpenGLRenderContext");
    LOG_INFO_C("OpenGL Version: " + std::string(reinterpret_cast<const char*>(version)), "OpenGLRenderContext");
    LOG_INFO_C("GLSL Version: " + std::string(reinterpret_cast<const char*>(glslVersion)), "OpenGLRenderContext");
...
std::shared_ptr<RenderShader> OpenGLRenderContext::createShader(const std::string& vertexCode, const std::string& fragmentCode) {
    ...
    if (!success) {
        ...
        LOG_ERROR_C("Vertex shader compilation failed: " + std::string(infoLog), "OpenGLRenderContext");
    ...
    if (!success) {
        ...
        LOG_ERROR_C("Fragment shader compilation failed: " + std::string(infoLog), "OpenGLRenderContext");
        glDeleteShader(vertexShader);
    ...
    if (!success) {
        ...
        LOG_ERROR_C("Shader program linking failed: " + std::string(infoLog), "OpenGLRenderContext");
        glDeleteShader(vertexShader);
        glDeleteShader(fragmentShader);
        glDeleteProgram(program);
        return nullptr;
...
bool OpenGLRenderContext::initializeGLEW() {
    ...
    GLenum err = glewInit();
    if (err != GLEW_OK) {
        LOG_ERROR_C("GLEW initialization failed: " + std::string(reinterpret_cast<const char*>(glewGetErrorString(err))), "OpenGLRenderContext");
        return false;
    }
...
void OpenGLRenderContext::checkGLErrors(const char* context) {
    ...
    while ((err = glGetError()) != GL_NO_ERROR) {
        ...
        LOG_ERROR_C("OpenGL error in " + std::string(context) + ": " + error, "OpenGLRenderContext");
    }
}
