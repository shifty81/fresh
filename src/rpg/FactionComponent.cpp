...
FactionComponent::FactionComponent(const std::string& factionName) 
...
FactionRelation FactionComponent::reputationToRelation(float reputation) const {
    if (reputation <= -50.0f) return FactionRelation::Hostile;
    if (reputation <= -10.0f) return FactionRelation::Unfriendly;
    if (reputation <= 10.0f) return FactionRelation::Neutral;
    if (reputation <= 50.0f) return FactionRelation::Friendly;
